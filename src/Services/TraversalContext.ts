...
import { IBasicBlock } from "../Models/IBasicBlock";
  ...
  export function addSuccessEdge(from: IBasicBlock | null, to: IBasicBlock | null) {
  ...
  export function addExceptionEdge(from: IBasicBlock | null, to: IBasicBlock | null) {
    if (!from || !to) throw new Error(`invalid edge ${from} -> ${to}`);
  ...
  export function splitCurrent(ctx: ITraversalContext): IBasicBlock {
